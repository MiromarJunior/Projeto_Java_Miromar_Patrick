...
public class CdtrPessoaDAO {
    ...
    public void update(CdtrPessoa cdtrPessoa){
        ...
        */
         ...
         sql = "UPDATE CDTR_PESSOA SET CAFU_PK = ?,CURS_PK = ?,"
    ...
    public void salvar(CdtrPessoa cdtrPessoa){
        if(cdtrPessoa.getPessPk()!=0 && cdtrPessoa!=null){
            update(cdtrPessoa);
        }else{
            insert(cdtrPessoa);
        }
     ...
     * @return */
      public List<CdtrPessoa> getByAll(){
        ...
        sql = "                 SELECT PESS_PK, CAFU_DESCRICAO AS FUNCAO,"
                + "             PESS_LOGIN AS LOGIN," +
...
"                               AND CDTR_PESSOA.CURS_PK = CDTR_CURSO.CURS_PK";
        ...
        try {
            prep = con.prepareStatement(sql);
              ...
              while(res.next()){
                  ...
                  pessoa.setPessPk(res.getInt("PESS_PK"));
    ...
    public List<CdtrPessoa> getByName(String nome){
        List<CdtrPessoa> lista = new ArrayList<>();
       sql =  "SELECT CAFU_DESCRICAO AS FUNCAO,PESS_LOGIN AS LOGIN,"
               + "CURS_DESCRICAO AS CURSO,PESS_NOME AS NOME, "
               + "PESS_LATTES AS LATTES "
               + "FROM CDTR_PESSOA,CDTR_CURSO,CDTR_FUNCAO "
               + "WHERE PESS_NOME LIKE ? "
               + "AND CDTR_PESSOA.CAFU_PK = CDTR_FUNCAO.CAFU_PK "
               + "AND CDTR_PESSOA.CURS_PK = CDTR_CURSO.CURS_PK ";
				;
       // sql = "SELECT * FROM CDTR_PESSOA WHERE PESS_NOME LIKE ?";
        ...
        try {
            ...
            while(res.next()){
                 ...
                 cdtrPessoa.setPessNome(res.getString("NOME"));
                cdtrPessoa.setCafuDescricao(res.getNString("FUNCAO"));
                 cdtrPessoa.setPessLogin(res.getString("LOGIN"));
                 lista.add(cdtrPessoa);
            }
   ...
   public CdtrPessoa getById(int id){
       CdtrPessoa pessoa = null;
...
"                               PESS_LATTES AS LATTES " +
"				FROM CDTR_PESSOA,CDTR_CURSO,CDTR_FUNCAO " +
"				WHERE PESS_PK = ? GROUP BY CDTR_PESSOA.PESS_NOME";
